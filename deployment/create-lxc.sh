...
$STD apt update
$STD apt install -y python3.12 python3.12-venv python3-pip nodejs npm nginx sqlite3 curl git
...
$STD python3.12 -m venv "$APP_DIR/.venv"
...
if [ ! -f "$APP_DIR/.env" ]; then
  $STD python3.12 -c "from cryptography.fernet import Fernet; print(f'SECRET_KEY={Fernet.generate_key().decode()}')" > "$APP_DIR/.env"
