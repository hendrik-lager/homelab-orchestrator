...
apt-get update
apt-get install -y python3.11 python3.11-venv python3-pip nodejs npm nginx sqlite3 curl git
...
if [ ! -f "$APP_DIR/.env" ]; then
    python3.11 -c "from cryptography.fernet import Fernet; print(f'SECRET_KEY={Fernet.generate_key().decode()}')" > "$APP_DIR/.env"
    cat >> "$APP_DIR/.env" <<'EOF'
